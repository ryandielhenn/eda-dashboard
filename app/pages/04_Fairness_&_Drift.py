...
# ───────────────────────────────────────────────
# Get current active dataset (from Explore)
# ───────────────────────────────────────────────
...
# Self-heal if session is empty or stale
...
with tab_fair:
    ...
    with c2:
        thresh = st.number_input(
            "Threshold",
            value=float(df[tcol].median()) if num_cols else 0.0
        )
    with c3:
        positive_def = st.selectbox(
            "Positive if",
            [f"{tcol} > {thresh}", f"{tcol} <= {thresh}"]
        )
    ...
    if num_cols and cat_cols and tcol in df and sattr in df:
        ...
        st.dataframe(
            tbl.sort_values("selection_rate", ascending=False),
            use_container_width=True,
        )
        st.caption(
            "This demo mimics Fairlearn's selection rate by group; "
            "in the full version we'll compute real fairness metrics via Fairlearn/Evidently."
        )
    else:
...
with tab_drift:
    ...
    ref = st.selectbox("Reference dataset", tables, key="ref_ds")
    cur = st.selectbox("Current dataset", tables, key="cur_ds")
